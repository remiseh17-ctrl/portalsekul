...
@section('page_class', 'page-guru-absensi')
@section('content')
    ...
    <div class="container-fluid px-4 py-8 mt-4">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 transition-colors duration-300">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p class="text-gray-600 dark:text-gray-300 mt-1">Kelola absensi siswa dengan mudah dan terintegrasi</p>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="text-sm text-gray-500 dark:text-gray-300 hidden sm:flex items-center">
                            ...
                            {{ $absensis->total() }} Data Absensi
                        </div>
                        <a href="{{ route('absensi.create') }}"
                           class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-teal-500 to-cyan-600 text-white rounded-lg shadow-lg hover:shadow-xl transform hover:-translate-y-1 hover:scale-105 transition-all duration-300 font-medium">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg dark:shadow-gray-900/20 border border-gray-200 dark:border-gray-700 overflow-hidden transition-transform duration-300 hover:-translate-y-1 hover:shadow-2xl">
            ...
            <div class="p-6 border-b border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-800/50 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div class="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
                    <div class="relative">
                        <input type="text"
                            id="liveSearchInput"
                            class="w-[150px] md:w-[180px] pl-9 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-teal-500 dark:focus:ring-teal-400 focus:border-teal-500 dark:focus:border-teal-400 transition-all duration-200 shadow-sm"
            ...
            <div class="overflow-x-auto">
                <table id="tableAbsensiGuru" class="w-full text-sm">
                    <thead class="bg-gray-100 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                        <tr>
                            <th class="px-4 py-3 text-center font-semibold text-gray-900 dark:text-white w-16">
                                <span class="flex items-center justify-center">
                                    <i data-lucide="hash" class="w-4 h-4 text-teal-500 dark:text-teal-400 mr-1"></i>
                                    No
                                </span>
                            </th>
                            ...
                            <th class="px-4 py-3 text-left font-semibold text-gray-900 dark:text-white">
                                <span class="flex items-center">
                                    <i data-lucide="check-circle" class="w-4 h-4 text-indigo-500 dark:text-indigo-400 mr-1"></i>
                                    Status
                                </span>
                            </th>
                        ...
                        <tr class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200">
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="font-semibold text-gray-900 dark:text-white">{{ $absensi->siswa->nama ?? '-' }}</span>
                                        <span class="text-xs text-gray-500 dark:text-gray-400">{{ $absensi->siswa->nis ?? '-' }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-4">
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-gray-900 dark:text-white font-medium">{{ $absensi->tanggal->format('d/m/Y') }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-4">
                                @php
                                ...
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold border {{ $statusColor }}">
                                    <i data-lucide="{{ $absensi->status == 'Hadir' ? 'check-circle' : ($absensi->status == 'Sakit' ? 'thermometer' : ($absensi->status == 'Izin' ? 'file-text' : 'x-circle')) }}" class="w-3 h-3 mr-1"></i>
                                    {{ $absensi->status }}
                                </span>
                            </td>
                            <td class="px-4 py-4">
                                <div class="flex justify-center gap-2">
                                    <button type="button"
                                            ...
                                            title="Edit Absensi" data-bs-toggle="modal" data-bs-target="#modalEditAbsensiGuru{{ $absensi->id }}">
                        ...
                        <tr class="bg-white dark:bg-gray-800">
                            <td colspan="7" class="px-4 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-400 dark:text-gray-500 text-sm mt-1">Mulai dengan menambahkan absensi terintegrasi (guru + siswa)</p>
                                    <a href="{{ route('absensi.create') }}"
                                       class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-teal-500 to-cyan-600 text-white rounded-lg shadow-lg hover:shadow-xl transform hover:-translate-y-1 hover:scale-105 transition-all duration-300 font-medium mt-4">
                                        <i data-lucide="plus" class="w-5 h-5"></i>
                                        <span>Tambah Absensi Pertama</span>
                                    </a>
                                </div>
                            </td>
                        </tr>
